...
type Platform = "instagram-feed" | "instagram-story" | "facebook-feed" | "twitter-feed" | "youtube" | "pinterest";
...
export default function AdPreviewPage() {
  ...
  const platforms = [
    ...
    { id: "pinterest", label: "Pinterest Pin", icon: (props: any) => (
      <svg width={props.size || 14} height={props.size || 14} style={props.style} viewBox="0 0 24 24" fill="currentColor">
  ...
  const renderAdPreview = () => {
    ...
    if (selectedPlatform === "pinterest") return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} style={commonStyle}>
        <div style={{ padding: "12px 14px", display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#bd081c", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 11 }}>P</div>
          <div>
            <div style={{ fontSize: 13, fontWeight: 700 }}>vulpinix.ai</div>
            <div style={{ fontSize: 11, color: "var(--vx-text-muted)" }}>Sponsored Pin</div>
          </div>
        </div>
        <div style={{ aspectRatio: "2/3", background: "var(--vx-bg-input)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
          {previewImage ? (
            <img src={previewImage} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt="Ad" />
          ) : (
            <div style={{ textAlign: "center", color: "var(--vx-text-muted)" }}>
              <Globe size={28} style={{ marginBottom: 8, opacity: 0.3 }} />
              <div style={{ fontSize: 11 }}>Pinterest Pin</div>
            </div>
          )}
        </div>
        <div style={{ padding: "14px", background: "var(--vx-bg-card)" }}>
          <h4 style={{ margin: "0 0 6px 0", fontSize: 14, fontWeight: 800 }}>{campaignData.name || "Ad Campaign"}</h4>
          <p style={{ fontSize: 12, lineHeight: 1.5, margin: 0, color: "var(--vx-text-secondary)" }}>
            {adCreative.caption.substring(0, 150)}...
          </p>
        </div>
        <button style={{ width: "100%", padding: "14px", background: "#bd081c", color: "#fff", border: "none", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
          {adCreative.cta}
        </button>
      </motion.div>
    );
