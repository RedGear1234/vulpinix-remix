            ...
            {campaigns.length>0&&(
              <motion.div {...FV} transition={{delay:0.18}} className="vxan-filter-bar">
                <div className="vxan-tabs">
                  {([{key:"recent",label:"Recent",icon:"🕐"},{key:"week",label:"Previous 7 Days",icon:"📅"},{key:"earlier",label:"Earlier",icon:"🗂️"}] as const).map(t=>(
                ...
                <div className="vxan-search-wrap">
                  ...
                  <input className="vxan-search" placeholder="Search campaigns…" value={search} onChange={e=>setSearch(e.target.value)}/>
                </div>
              </motion.div>
            )}
